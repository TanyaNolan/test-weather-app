...
class NatlexTest : TestCase(
    ...
    fun natlextest() = before {
        ...
        val NatlexTestButton: UiObject = mDevice.findObject(UiSelector().description("NatlexTest"))
        NatlexTestButton.clickAndWaitForNewWindow()
    }.after {

    }.run {
        NatlexTest.NatlextestScreen {
            val city = "Petrozavodsk"
            ...
            step("2. Check that the city appears in the main card with name, temperature and color") {
                weather.isDisplayed()
                searchedCity.hasText(city) //check name
                searchedTemp.hasAnyText() //check temp
                switch.setChecked(true) //turn on C
                //to check color take a screenshot and get pixel color by bitmap
                val path = "/sdcard/weather.png"
